        ...
        switch ($_POST['action']) {
            case 'add':
                ...
                try {
                    ...
                    if ($email !== '') {
                        $chk = $pdo->prepare('SELECT COUNT(*) FROM faculty WHERE email = ?');
                        $chk->execute([$email]);
                        if ((int)$chk->fetchColumn() > 0) {
                            $_SESSION['flash_error'] = 'A faculty member with this email already exists.';
                            header('Location: manage_faculty.php');
                            exit();
                        }
                    }

                    // Duplicate check by name+department+specialization
                    $chk2 = $pdo->prepare('SELECT COUNT(*) FROM faculty WHERE professor_name = ? AND department = ? AND specialization = ?');
                    $chk2->execute([$name, $dept, $spec]);
                    if ((int)$chk2->fetchColumn() > 0) {
                        $_SESSION['flash_error'] = 'This faculty member already exists in the same department and specialization.';
                        header('Location: manage_faculty.php');
                        exit();
                    }
                    ...
                    $stmt->execute([$name, $dept, $spec, $email ?: null, $contact ?: null]);

                    $_SESSION['flash_message'] = 'Faculty member added successfully!';
                    header('Location: manage_faculty.php');
                    exit();
                } catch (PDOException $e) {
                    $_SESSION['flash_error'] = 'Error adding faculty member: ' . $e->getMessage();
                    header('Location: manage_faculty.php');
                    exit();
                }
                break;
            ...
            case 'edit':
                try {
                    ...
                    $_SESSION['flash_message'] = "Faculty member updated successfully!";
                    header('Location: manage_faculty.php');
                    exit();
                } catch (PDOException $e) {
                    $_SESSION['flash_error'] = "Error updating faculty member: " . $e->getMessage();
                    header('Location: manage_faculty.php');
                    exit();
                }
                break;
            ...
            case 'delete':
                try {
                    ...
                    $_SESSION['flash_error'] = "Error deleting faculty member: " . $e->getMessage();
                    header('Location: manage_faculty.php');
                    exit();
                }
                break;
